...
``scheduler_components/setup/kv_cache.py`` (created by
...
SUBJECT = (
    "Extract _maybe_register_hicache_draft to scheduler_components/setup/kv_cache.py"
)
BODY = """\
Move ``_maybe_register_hicache_draft`` off Scheduler into a free function
``maybe_register_hicache_draft`` in the same
``scheduler_components/setup/kv_cache.py`` introduced for
